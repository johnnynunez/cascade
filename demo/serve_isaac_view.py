...
class RecoveringViewRig:
    ...
    def state(self):
        with self._lock:
            ...
            status=("OpenClaw Demo - live" if online else
                    "OpenClaw Demo - waiting for fresh cameras" if self._online else
                    "OpenClaw Demo - offline; reconnecting")
    ...
    def _validate(self,frame,camera):
        cap=getattr(frame,"capture",None) or {}; prop=cap.get("proprioception") or {}
        stamp=cap.get("t")
        if (type(stamp) not in (float,int) or not math.isfinite(stamp)
                or stamp < 0 or self._clock()-stamp < -.25):
            raise RuntimeError(f"{camera}: capture timestamp missing or invalid")
        ...
        # A slow render is not a broken connection. Keep polling the existing
        # transport; latest()/stats() hide each stale camera independently.
        # Reconnecting here used to discard healthy views and delay recovery
        # with backoff whenever just one render crossed the freshness limit.
    def _run(self):
        delay=self._initial
        try:
