...
def test_staff_page_is_separate_authenticated_and_cannot_proxy_requests(camera):
    with serving(visitor.VisitorHandler) as (server, origin):
        ...
        for path in ("/staff/media/unknown.png", "/staff/media/../visitor.py",
                     "/staff/media/%2e%2e/visitor.py", "/staff/media/auth.json"):
            assert fetch(origin + path, headers=headers)[0] == 404
    assert camera[1] == []
