...
def setup(urls_file=None, sso_confirmed=False, sso_issuer=None, sso_audience=None):
    ...
    # Bound inference time independently of tool execution.
    ...
    config = {'gateway': {'mode': 'local', 'port': 18790, 'bind': 'loopback',
        ...
        'agents': {'defaults': {'workspace': str(workspace), 'model': {'primary': 'cascade-gpu/' + MODEL_ID},
                                ...
                                # Leave room for robot tools and new camera images.
                   ...
                   'entries': {'main': {'name': 'main', 'workspace': str(workspace),
                     ...
                     # The native image estimate can clamp replies to one token.
                     # Keep a bounded reply budget; the server checks its real context.
                     'model': 'cascade-gpu/' + MODEL_ID,
                     'params': {'temperature': 0, 'extra_body': {'max_completion_tokens': 1024}}}}},
        'tools': {'profile': 'coding', 'allow': ['cascade__*']},
        # Retire idle perception workers between visitors. OpenClaw keeps
        # active tool leases alive and recreates the runtime on the next call.
        'mcp': {'sessionIdleTtlMs': 60000, 'servers': {'cascade': {'command': env['PAAI_APP_PYTHON'],
