...
After one automatic runtime restart, Worktop video reconnected at
640 × 360, the size used for offline images, while snapshots had returned
to 960 × 540.
Replacing only that encoder restored the full video size; the application
container and visitor service kept running. Resolution recovery across a
future source restart still needs separate verification.
...
The bounded live object matrix passed for orange and tomato can. Lemon failed
to lift; orange-on-can failed stability and was safely reset. These are
recorded limitations, not additional successful manipulation cases. The
unpacked Chrome extension passed on Brev in its camera-companion scope.
...
For booth support, contact Johnny Núñez or Asier Arranz on Slack.
