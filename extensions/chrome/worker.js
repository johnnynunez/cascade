...
chrome.runtime.onMessage.addListener((message, sender, reply) => {
  ...
  if (message.type === 'open-sidepanel') {
    ...
    chrome.sidePanel.open(options).then(async () => {
      if (sender.tab) {
        await chrome.storage.session.remove(`allowed:${sender.tab.id}`);
  ...
  if (['collapse','close','move','enlarge-camera'].includes(message.type) && sender.tab) {
    if (message.type === 'close') chrome.storage.session.remove(`allowed:${sender.tab.id}`);
