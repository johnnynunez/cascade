"""CASCADE_BOOTH tuning overlay: configs/booth.yaml deep-merged over demo.yaml
...
def test_booth_env_falsey_values_disable(monkeypatch):
    for v in ("0", "false", "False", "", "0 ", " false", "no", "off", "OFF"):
        monkeypatch.setenv("CASCADE_BOOTH", v)
        cfg = load_demo_config()
        assert "booth_mode" not in cfg, f"CASCADE_BOOTH={v!r} must not enable booth"
...
def test_booth_env_with_missing_overlay_fails_loud(monkeypatch, tmp_path):
    """An explicitly requested overlay must never no-op silently: CASCADE_BOOTH
