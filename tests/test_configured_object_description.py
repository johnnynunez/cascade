...
@pytest.mark.parametrize("query", ["lemon", "the lemon", "a lemon", " THE LEMON "])
def test_named_lemon_uses_current_yellow_pixels_without_relabeling(demo_cfg, tmp_path, query):
    runtime, _ = runtime_for([(0, 255, 255), (0, 255, 0)], demo_cfg, tmp_path)
    result = runtime.skill_localize_object(query)
    ...
    assert result["target_resolution"] == {
        "requested": query, "visual_query": "yellow object", "detected_as": "lime",
...
@pytest.mark.parametrize("query", ["lemon", "the lemon", "a lemon"])
def test_two_yellow_objects_refuse_instead_of_ranking(demo_cfg, tmp_path, query):
    runtime, _ = runtime_for([(0, 255, 255)] * 2, demo_cfg, tmp_path)
    with pytest.raises(SkillError, match="2 reachable objects"):
        runtime.skill_localize_object(query)
...
@pytest.mark.parametrize("query", ["lemon", "the lemon"])
def test_missing_current_evidence_cannot_use_memory_or_vlm(demo_cfg, tmp_path, missing, query):
...
def test_description_does_not_override_other_queries_or_references(demo_cfg, tmp_path):
    ...
    for query in ("green cube", "the second lemon", "a different lemon",
                  "the lemon nearest the can", "the lemon not the yellow one"):
        assert runtime._visual_query(query) == query


def test_exact_configured_name_precedes_article_removal(demo_cfg, tmp_path):
    runtime, _ = runtime_for([(0, 255, 255)], demo_cfg, tmp_path)
    runtime.cfg._data["object_descriptions"]["the lemon"] = "green object"
    assert runtime._visual_query("the lemon") == "green object"
    assert runtime._visual_query("a lemon") == "yellow object"


@pytest.mark.parametrize("query", ["the orange", "an orange", " THE\tORANGE "])
def test_article_uses_the_configured_orange_description(demo_cfg, tmp_path, query):
    runtime, _ = runtime_for([(0, 255, 255)], demo_cfg, tmp_path)
    runtime.cfg._data["object_descriptions"]["orange"] = "orange fruit"
    assert runtime._visual_query(query) == "orange fruit"


@pytest.mark.parametrize("description", [None, "fruit", "", 123, {"position": [1, 2, 3]}])
