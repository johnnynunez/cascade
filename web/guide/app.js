/* Booth connection page: read-only status and live views. The launch client owns actions. */
(function(root){
  'use strict';
  ...
  function render(){
    ...
    $('status-detail').textContent=snapshot.ready?'Open OpenClaw, then put the camera extension beside the chat.':snapshot.stopped?'Refresh this guide and wait for the booth operator to restore the demo.':snapshot.needsAttention?'Refresh this guide and check the status before sending a movement.':'Click Open OpenClaw to follow the connection checks.';
